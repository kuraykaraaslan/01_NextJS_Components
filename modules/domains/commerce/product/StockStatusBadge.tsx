...
import type { StockStatus } from '@/modules/domains/commerce/types';

const stockMeta: Record<StockStatus, { label: string; variant: 'success' | 'warning' | 'error' | 'info' }> = {
...
export function StockStatusBadge({ status, size = 'md', className }: StockStatusBadgeProps) {
  const meta = stockMeta[status] ?? { label: status, variant: 'info' as const };
  return (
