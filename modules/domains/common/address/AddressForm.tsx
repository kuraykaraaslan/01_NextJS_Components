...
/* Form uses plain strings internally; Address type has nullable fields. */
type FormValues = {
  fullName: string;
  phone: string;
  addressLine1: string;
...
function fromAddress(a?: Partial<Address>): FormValues {
  return {
    fullName:     toStr(a?.fullName),
    phone:        toStr(a?.phone),
    addressLine1: toStr(a?.addressLine1),
    addressLine2: toStr(a?.addressLine2),
    city:         toStr(a?.city),
    state:        toStr(a?.state),
    postalCode:   toStr(a?.postalCode),
    country:      toStr(a?.country),
    countryCode:  toStr(a?.countryCode),
  };
}
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className={cn('space-y-4', className)}>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input
          id="addr-fullname" label="Full Name" required
        ...
        />
        <Input
          id="addr-phone" label="Phone" type="tel"
      ...
      <Input
        id="addr-line1" label="Address Line 1" required
      ...
      />
      <Input
        id="addr-line2" label="Address Line 2 (optional)"
